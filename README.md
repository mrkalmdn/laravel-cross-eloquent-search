...
_Note: in previous versions of this package, this method was called `startWithWildcard()`._
...
If you search for _Apple iPad_, the second sentence will come up first, as there are more matches of the search terms.
...
Ordering by relevance is _not_ supported if you're searching through (nested) relationships.
...
You can search through (nested) relationships by using the _dot_ notation:
...
MySQL has a _soundex_ algorithm built-in so you can search for terms that sound almost the same. You can use this feature by calling the `soundsLike` method:
